  ...
  return (
    <section className="section">
      <div className="container" style={{display:'grid', gap:24}}>
        <h2>Contact</h2>
        <motion.form initial={{opacity:0, y:16}} whileInView={{opacity:1, y:0}} viewport={{once:true}} transition={{duration:0.5}} className="glass" style={{borderRadius:20, padding:'28px 22px', display:'grid', gap:16, background:'rgba(10,20,50,0.35)'}} onSubmit={(e)=>e.preventDefault()}>
          <label>
          ...
          <label>
            ...
            <textarea required name="message" rows="6" placeholder="How can we help?" className="glass" style={{width:'100%', padding:'12px 14px', borderRadius:12, resize:'vertical', color:'#eaf2ff', background:'rgba(255,255,255,0.06)'}} />
          </label>
          ...
          <small style={{color:'#9fb7e6'}}>Email: grss-muj@ieee.org</small>
        </motion.form>

        <div style={{display:'flex', gap:12}}>
          <Social href="https://www.linkedin.com" label="LinkedIn"><FaLinkedin /></Social>
          <Social href="https://www.instagram.com" label="Instagram"><FaInstagram /></Social>
          <Social href="https://twitter.com" label="Twitter"><FaTwitter /></Social>
        </div>
      </div>
    </section>
  );
