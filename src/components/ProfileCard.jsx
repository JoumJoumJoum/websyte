...
const ProfileCardComponent = ({
  ...
  const handlePointerEnter = useCallback(() => {
    ...
    wrap.style.setProperty('--behind-gradient', 'radial-gradient(farthest-side circle at var(--pointer-x) var(--pointer-y),hsla(210,100%,80%,var(--card-opacity)) 4%,hsla(210,50%,70%,calc(var(--card-opacity)*0.75)) 10%,hsla(210,25%,60%,calc(var(--card-opacity)*0.5)) 50%,hsla(210,0%,60%,0) 100%),radial-gradient(35% 52% at 55% 20%,#00ffaac4 0%,#073aff00 100%),radial-gradient(100% 100% at 50% 50%,#00c1ffff 1%,#073aff00 76%),conic-gradient(from 124deg at 50% 50%,#07c6ffff 0%,#c137ffff 40%,#07c6ffff 60%,#c137ffff 100%)');
    wrap.style.setProperty('--inner-gradient', 'linear-gradient(145deg,#1a27488c 0%,#71C4FF44 100%)');
  }, [animationHandlers]);
  ...
  const handleDeviceOrientation = useCallback((event) => {
    const card = cardRef.current; const wrap = wrapRef.current; if (!card || !wrap || !animationHandlers) return;
    const { beta, gamma } = event; if (!beta || !gamma) return;
  ...
  useEffect(() => {
    ...
    const pointerMoveHandler = handlePointerMove; const pointerEnterHandler = handlePointerEnter; const pointerLeaveHandler = handlePointerLeave; const deviceOrientationHandler = handleDeviceOrientation;

    const handleClick = () => {
      if (!enableMobileTilt || window.location.protocol !== 'https:') return;
      if (typeof window.DeviceMotionEvent.requestPermission === 'function') {
        window.DeviceMotionEvent.requestPermission().then(state => { if (state === 'granted') window.addEventListener('deviceorientation', deviceOrientationHandler); }).catch(() => {});
    ...
    const initialX = wrap.clientWidth - ANIMATION_CONFIG.INITIAL_X_OFFSET; const initialY = ANIMATION_CONFIG.INITIAL_Y_OFFSET;
      ...
      <section ref={cardRef} className="pc-card">
        <div className="pc-inside">
          ...
          <div className="pc-content pc-avatar-content">
            <img className="avatar" src={avatarUrl} alt={`${name || 'User'} avatar`} loading="lazy" onError={(e) => { const t = e.target; t.style.display = 'none'; }} />
            {showUserInfo && (
              <div className="pc-user-info">
                <div className="pc-user-details">
                  <div className="pc-mini-avatar">
                    <img src={miniAvatarUrl || avatarUrl} alt={`${name || 'User'} mini avatar`} loading="lazy" onError={(e) => { const t = e.target; t.style.opacity = '0.5'; t.src = avatarUrl; }} />
                  </div>
